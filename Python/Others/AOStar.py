...
def get_node(mark_road, extended):
    ...
    while 1:
        ...
        if current not in extended:  # Find nodes that have not been expanded
            ...
            for child in mark_road[current]:  # If the current node has been expanded, then use the idea of ​​dfs and add the children of its marked road to
...
def AOstar(nodes_tree, h_val):
    ...
    time.sleep(1)
    while not solved[0] and h_val[0] < futility:  # When the initial node is unsolved and does not exceed the threshold
        node = get_node(mark_rode, extended)   # Find an unexpanded node on the sign road
        ...
        for suc_edge in nodes_tree[node]:  # Generate node's successor node
            for suc_node in suc_edge:
                if nodes_tree[suc_node] == [[suc_node]]:  # If it is a solution, it is marked as solved
                    solved[suc_node] = True
        S = [node]
        while S:  # When S is not empty
            current = get_current(S, nodes_tree)  # Find current where the successor node is not in S
            S.remove(current)
            origen_h = h_val[current]
            origen_s = solved[current]

            min_h = 0xfff
            for edge in nodes_tree[current]:  # Traverse the current shortest subpath
                edge_h = 0
                for node in edge:
                    edge_h += h_val[node] + 1   # Cost of child node + path 1 fee
                if edge_h < min_h:
                    min_h = edge_h
                    h_val[current] = min_h
            ...
            if mark_rode[current] not in choice:
                ...
                time.sleep(1)
            for child_node in mark_rode[current]:  # The predecessor of each node in the subpath is set to current
                pre[child_node].append(current)
            solved[current] = True  # If each node of the subpath is solved, then current is also set to solved
